...
import { z } from "zod";
import { LoaderCircle, Upload, X, Tag, Palette, Package, DollarSign, ImageIcon, FileText, ChevronRight, Percent } from "lucide-react";
...
const CATEGORY_GROUPS = {
  moda: { label: "Moda y Accesorios", emoji: "👕", categories: ["Ropa y Accesorios", "Calzado", "Joyería y Relojes"], hasSizes: true, hasColors: true },
  belleza: { label: "Belleza y Bienestar", emoji: "✨", categories: ["Belleza y Cuidado Personal", "Salud y Bienestar", "Bebés"], hasSizes: false, hasColors: true },
  hogar: { label: "Hogar y Arte", emoji: "🏠", categories: ["Hogar y Jardín", "Arte y Manualidades"], hasSizes: false, hasColors: true },
  deportes: { label: "Deportes y Tecnología", emoji: "⚽", categories: ["Deportes y Fitness", "Electrónica", "Juguetes y Juegos", "Vehículos y Accesorios"], hasSizes: false, hasColors: false },
  otros: { label: "Alimentos y Otros", emoji: "🍎", categories: ["Alimentos y Bebidas", "Libros y Papelería", "Mascotas", "Otros"], hasSizes: false, hasColors: false },
} as const;
...
export function AddProductModal({ isOpen, onClose, storeId }: AddProductModalProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { name: "", description: "", costPrice: 0, price: 0, stock: 0, category: "", onSale: false, salePrice: 0, saleLabel: "Oferta" },
  });
  ...
  useEffect(() => {
    if (!hasSizes) setSelectedSizes([]);
    if (!hasColors) setSelectedColors([]);
  }, [hasSizes, hasColors]);

  const handleClose = () => {
    form.reset({ name: "", description: "", costPrice: 0, price: 0, stock: 0, category: "", onSale: false, salePrice: 0, saleLabel: "Oferta" });
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      form.setValue("image", undefined); setImagePreview(null); e.target.value = ""; return;
    }
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    if (!user || !firestore || !storeId) {
      toast({ variant: "destructive", title: "Error", description: "No se pudo guardar. Intenta de nuevo." });
      return;
    }
    setIsLoading(true);

    const productId = uuidv4();
    let imageUrl = `https://picsum.photos/seed/${productId}/400/400`;
    ...
    if (data.image && data.image.length > 0) {
      try {
        ...
        toast({ variant: "destructive", title: "Error al procesar imagen", description: "Se usará una imagen de marcador." });
      }
    }

    const productData = {
      id: productId, storeId, ownerId: user.uid,
      name: data.name, description: data.description,
      costPrice: data.costPrice, price: data.price,
      stock: data.stock, category: data.category,
      imageUrl, imageHint: "imagen de producto",
    ...
    setDoc(ref, productData).catch(() => {
      errorEmitter.emit('permission-error', new FirestorePermissionError({ path: ref.path, operation: 'create', requestResourceData: productData }));
    });

    toast({ title: "¡Producto añadido!", description: `${data.name} ya está en tu tienda.` });
    setIsLoading(false);
    handleClose();
  };

  const margin = watchPrice > 0 && form.watch("costPrice") >= 0
    ? (((watchPrice - form.watch("costPrice")) / watchPrice) * 100).toFixed(0) : null;
  const saleDiscount = watchOnSale && watchPrice > 0 && (watchSalePrice || 0) > 0
    ? (((watchPrice - (watchSalePrice || 0)) / watchPrice) * 100).toFixed(0) : null;
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open && !isLoading) handleClose(); }}>
      <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden rounded-3xl border-0 shadow-2xl">
        <div className="bg-amber-700 px-6 py-5">
          <DialogTitle className="text-white font-black text-xl">Añadir Producto</DialogTitle>
          <DialogDescription className="text-amber-200 text-sm mt-0.5">Completa la información de tu nuevo producto</DialogDescription>
        </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="overflow-y-auto max-h-[72vh] px-6 py-5 space-y-5">
            ...
            <div className="grid gap-1.5">
              ...
              <Input {...form.register("name")} placeholder="Ej: Tenis Nike Air Max" className="rounded-xl h-11" disabled={isLoading} />
            ...
            <div className="grid gap-1.5">
              ...
              <Textarea {...form.register("description")} placeholder="Describe tu producto..." className="rounded-xl resize-none min-h-[80px]" disabled={isLoading} />
              {form.formState.errors.description && <p className="text-xs text-red-500">{form.formState.errors.description.message}</p>}
            </div>

            <SectionTitle icon={<ImageIcon className="h-4 w-4" />} title="Imagen del producto" />
            <div className="flex items-center gap-4">
              <div className="relative h-20 w-20 rounded-2xl overflow-hidden bg-gray-100 border-2 border-dashed border-gray-200 shrink-0 flex items-center justify-center">
                {imagePreview ? <img src={imagePreview} alt="Preview" className="h-full w-full object-cover" /> : <Upload className="h-6 w-6 text-gray-300" />}
              </div>
            ...
            <div className="grid gap-1.5">
              <Select onValueChange={(v) => form.setValue("category", v, { shouldValidate: true })} disabled={isLoading}>
                <SelectTrigger className="rounded-xl h-11"><SelectValue placeholder="¿Qué tipo de producto es?" /></SelectTrigger>
                <SelectContent className="rounded-2xl">
                  {Object.values(CATEGORY_GROUPS).map((group) => (
                    <div key={group.label}>
                      ...
                      {group.categories.map((cat) => <SelectItem key={cat} value={cat} className="pl-6">{cat}</SelectItem>)}
                    </div>
                  ))}
                </SelectContent>
              </Select>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {basicColors.map((color) => (
                        <button key={color} type="button" onClick={() => addColor(color)} disabled={selectedColors.includes(color)}
                          className={cn("text-xs px-2.5 py-1 rounded-lg border transition-all", selectedColors.includes(color) ? "bg-amber-100 border-amber-300 text-amber-700 cursor-default" : "bg-gray-50 border-gray-200 text-gray-600 hover:border-amber-400 hover:text-amber-700")}>
                          {color}
                        </button>
                      ))}
                    </div>
                    ...
                    {selectedColors.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {selectedColors.map((c) => (
                          <Badge key={c} className="bg-amber-50 text-amber-800 border border-amber-200 pr-1.5">{c}
                ...
                {hasSizes && (
                  <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {["XS", "S", "M", "L", "XL", "XXL", "XXXL"].map((size) => (
                          <button key={size} type="button" onClick={() => addSize(size)} disabled={selectedSizes.includes(size)}
                            className={cn("text-xs px-3 py-1 rounded-lg border font-medium transition-all", selectedSizes.includes(size) ? "bg-amber-100 border-amber-300 text-amber-700 cursor-default" : "bg-gray-50 border-gray-200 text-gray-600 hover:border-amber-400 hover:text-amber-700")}>
                            {size}
                          </button>
                        ))}
                      </div>
                        ...
                        <>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {["22", "23", "24", "25", "26", "27", "28", "29", "30"].map((size) => (
                              <button key={size} type="button" onClick={() => addSize(size)} disabled={selectedSizes.includes(size)}
                                className={cn("text-xs px-2.5 py-1 rounded-lg border font-medium transition-all", selectedSizes.includes(size) ? "bg-amber-100 border-amber-300 text-amber-700 cursor-default" : "bg-gray-50 border-gray-200 text-gray-600 hover:border-amber-400 hover:text-amber-700")}>
                                {size}
                              </button>
                            ))}
                          </div>
                    ...
                    {selectedSizes.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {selectedSizes.map((s) => (
                          <Badge key={s} className="bg-amber-50 text-amber-800 border border-amber-200 pr-1.5">{s}
            ...
            <div className="space-y-1.5">
              ...
              <Input type="number" {...form.register("stock")} className="rounded-xl h-11" disabled={isLoading} placeholder="0" />
              {form.formState.errors.stock && <p className="text-xs text-red-500">{form.formState.errors.stock.message}</p>}
            </div>

            {/* ── OFERTA ── solo visible si ya hay precio */}
            {watchPrice > 0 && (
              <>
                ...
                <div className="bg-orange-50 border border-orange-100 rounded-2xl p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch checked={watchOnSale || false} onCheckedChange={(v) => form.setValue("onSale", v)} disabled={isLoading} />
                  </div>
                  {watchOnSale && (
                    <div className="space-y-3 pt-2 border-t border-orange-100">
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">$</span>
                          <Input type="number" step="0.01" {...form.register("salePrice")} className="pl-7 rounded-xl h-11" disabled={isLoading} placeholder={`Menor a $${watchPrice}`} />
                        </div>
                        {saleDiscount && Number(saleDiscount) > 0 && (
                          <p className="text-xs text-emerald-600 font-semibold">
                            El cliente ahorra {saleDiscount}% — precio original ${watchPrice}
                          </p>
                        )}
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {SALE_LABELS.map((label) => (
                            <button key={label} type="button" onClick={() => form.setValue("saleLabel", label)}
                              className={cn("text-xs px-3 py-1.5 rounded-lg border font-semibold transition-all",
                                form.watch("saleLabel") === label ? "bg-orange-500 border-orange-500 text-white" : "bg-white border-gray-200 text-gray-600 hover:border-orange-400 hover:text-orange-600")}>
                              {label}
                            </button>
                          ))}
                        </div>
          ...
          <div className="border-t bg-gray-50/80 px-6 py-4 flex gap-3">
            <Button type="button" variant="outline" onClick={handleClose} disabled={isLoading} className="rounded-xl flex-1">Cancelar</Button>
            <Button type="submit" disabled={isLoading} className="rounded-xl flex-1 bg-amber-700 hover:bg-amber-800 text-white font-bold">
              {isLoading ? <><LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> Guardando...</> : <><ChevronRight className="mr-2 h-4 w-4" /> Guardar Producto</>}
            </Button>
          </div>
        </form>
      </DialogContent>
